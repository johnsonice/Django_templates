...
from yolo.yolomodel import predict
import os
...
def uploadimage(request,tfnet=tfnet):
    if request.method == "POST":
        form = forms.uploadimage(request.POST,request.FILES)
        if form.is_valid():
            #do something here
            print("Validation Success")
            result = handle_uploaded_file(request.FILES['image'],tfnet)
            #print(result)
...
def handle_uploaded_file(f,tfnet):
    print(os.getcwd())
    with open('static/images/in/test.jpg','wb+') as destination:
        for chunk in f.chunks():
            destination.write(chunk)
    ## preload weights
    print('it is still fine here!')
    result = predict.predict(tfnet)
    return result
